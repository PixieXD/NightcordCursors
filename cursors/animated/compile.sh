...
mkdir -p ./${NAME}_animated/cursors
cat <<EOF >>./${NAME}_animated/index.theme
[Icon Theme]
Name = ${NAME}_animated
Comment = $DESCRIPTION
...
for cursor in "${set_cursors[@]}"; do
  ...
  mv $cursor ../../${NAME}_animated/cursors
done

cd ../../${NAME}_animated/cursors
for cursor in "${set_cursors[@]}"; do
...
if [ "$hyprcursorEnable" == "true" ]; then
  hyprcursor-util -x ${NAME}_animated --resize nearest
  hyprcursor-util -c "extracted_${NAME}_animated"
  mv theme_Extracted\ Theme "${DIRNAME}Hyprcursor_animated"
  rm -r "extracted_${NAME}_animated"
  cat >./${DIRNAME}Hyprcursor_animated/manifest.hl <<EOF
name = ${DIRNAME}Hyprcursor_animated
